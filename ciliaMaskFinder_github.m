...
PARAMS.ciliaNum=1; %to specify the position of the cilia-marker channel
...
PARAMS.numFields=4; %to specify the number of fields captured in this Leica file
PARAMS.numMarkers=2; %to specify the number of markers searched for; usually just 3 because one staining each for DAPI, cilia, POI
%specify the leica file to analyze
leicaFile = '<FILENAME>.lif';
%generate output file name
PARAMS.output_name=leicaFile(1:end-4);
%get the max-Z projections for each marker
[fields,zstacks] = get_max_z_projections(leicaFile, PARAMS);
