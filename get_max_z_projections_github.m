...
function [fields,zstacks]=get_max_z_projections(leicaFile, PARAMS)

%add path to bio-formats folder
addpath('<PATH>/bfmatlab/');

%read in the Leica imaging file
data = bfopen(leicaFile);

fields = cell(PARAMS.numFields,15); %create a cell array to hold the max-z-projections from each field
zstacks = {};
...
display('Done with max-z-projections for all fields');
